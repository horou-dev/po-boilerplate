/**
  ...
  *
  * Adding new features into the original.
  *
  * Version: 1.0.0
  *
  * Credit: Horou, Pokemon Online developers.
**/
...
var Config = {
	base_url: "https://raw.githubusercontent.com/horou-dev/po-boilerplate/master/",
	dataDir: "server-data/",
	bot: "Dratini",
	kickbot: "Blaziken",
	capsbot: "Exploud",
	...
	Mafia: {
		bot: "Murkrow",
		norepeat: 5,
		stats_file: "server-data/mafia_stats.json",
		max_name_length: 16,
...
require = function require(module_name, retry) {
	...
	with (module) {
		var content = sys.getFileContent("server-scripts/"+module_name);
		if (content) {
			try {
				eval(sys.getFileContent("server-scripts/"+module_name));
				sys.writeToFile("server-scripts/" + module_name + "-b", sys.getFileContent("server-scripts/" + module_name));
			} catch(e) {
				if (staffchannel)
...
var updateModule = function updateModule(module_name, callback) {
	...
	if (/^https?:\/\//.test(module_name))
	  ...
	  url = base_url + "server-scripts/"+ module_name;
	var fname = module_name.split(/\//).pop();
	if (!callback) {
		var resp = sys.synchronousWebCall(url);
		if (resp === "") return {};
		sys.writeToFile("server-scripts/"+fname, resp);
...
for (var i = 0; i < deps.length; ++i) {
	if (!sys.getFileContent("server-scripts/"+deps[i])) {
...
SESSION.identifyScriptAs("PO Scripts v1.0.0");
...
if (typeof SESSION.global() != 'undefined') {
	SESSION.global().channelManager = new POChannelManager('server-data/channelHash.txt');
...
init : function() {
	...
	if (typeof nameWarns == 'undefined') {
		nameWarns = [];
		try {
			var serialized = JSON.parse(sys.getFileContent("server-data/nameWarns.json"));
...
refreshNamebans: function() {
	nameBans = [];
	try {
		var serialized = JSON.parse(sys.getFileContent("server-data/nameBans.json"));
